...
function detectCanvasMaxDimension() {
...
function getSelectedLayout() {
    const selected = document.querySelector('input[name="step1Layout"]:checked');
    return selected?.value ?? "tall";
}
function computeDims(N, M, layout) {
...
function tileIndexToRGB(t, N) {
    const r = t % N;
    const g = Math.floor(t / N) % N;
    const b = Math.floor(t / (N * N));
    return { r, g, b };
}
function tileCoords(t, N, layout) {
...
function setDownloadLink(a, blob, filename) {
...
async function loadPngToImageData(file) {
...
function detectGeometry(width, height) {
    ...
    return {
        error: "Could not infer N/M/layout from dimensions. The image may have been resized, cropped, padded, or otherwise changed.",
    };
}
function compilePixelFilter(userCode) {
  ...
  `;
    ...
    const fn = new Function("R", "G", "B", "X", "Y", "W", "H", wrapped);
    return fn;
}
async function renderStep1Preview(N, layout, canvas) {
    const tilesW = layout === "wide" ? N * N : N;
    const tilesH = layout === "wide" ? N : N * N;
...
async function generateStep1Pattern(N, M, layout, statusEl, onProgress, cancelRef) {
    ...
    if (width > CANVAS_MAX_DIM || height > CANVAS_MAX_DIM) {
        ...
        throw new Error(`Requested image is ${formatInt(width)}×${formatInt(height)} px. ` +
...
async function applyStep2Filter(img, width, height, filterFn, onProgress, cancelRef) {
...
function makePngFromImageData(img) {
    const canvas = document.createElement("canvas");
...
function sampleTileColor(data, imgW, imgH, tileX, tileY, M) {
    // Inset sampling to reduce edge bleed.
    let margin = Math.max(1, Math.floor(M * 0.2));
    if (margin * 2 >= M)
        margin = Math.max(0, Math.floor(M / 4));
...
async function generateCubeLut(img, geom, onProgress, cancelRef) {
...
function setStatus(el, msg, tone = "normal") {
    el.textContent = msg;
    el.style.color =
        tone === "error"
...
function validateN(N) {
    ...
    if (N > 65)
        return "N above 65 is uncommon and can create very large images. (This tool caps UI N to 65.)";
    return null;
}
function validateM(M) {
    ...
    if (M > 512)
        return "M is very large and may exceed canvas limits.";
    return null;
}
document.addEventListener("DOMContentLoaded", () => {
    ...
    const step1GenerateBtn = $("step1GenerateBtn");
    const step1DownloadLink = $("step1DownloadLink");
    ...
    async function refreshStep1UI() {
        ...
        if (errN)
            setStatus(step1Status, errN, "warn");
        else if (errM)
            setStatus(step1Status, errM, "warn");
        else
            setStatus(step1Status, "", "normal");
        const { tilesW, tilesH, width, height } = computeDims(N, M, layout);
    ...
    document.querySelectorAll('input[name="step1Layout"]').forEach((r) => {
    ...
    step1GenerateBtn.addEventListener("click", async () => {
        ...
        const errM = validateM(M);
        if (errN)
            return setStatus(step1Status, errN, "error");
        if (errM && M > 0)
            setStatus(step1Status, errM, "warn");
        step1CancelRef = { cancelled: false };
        step1GenerateBtn.disabled = true;
        try {
            const blob = await generateStep1Pattern(N, M, layout, step1Status, (p) => {
    ...
    const step2LoadScriptBtn = $("step2LoadScriptBtn");
    const step2SaveScriptBtn = $("step2SaveScriptBtn");
    const step2RunBtn = $("step2RunBtn");
    const step2CancelBtn = $("step2CancelBtn");
    const step2Progress = $("step2Progress");
    const step2ProgressLabel = $("step2ProgressLabel");
    const step2DownloadLink = $("step2DownloadLink");
    ...
    step2RunBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            const out = await applyStep2Filter(imgCopy, width, height, fn, (p) => {
    ...
    const step3GenerateBtn = $("step3GenerateBtn");
    const step3CancelBtn = $("step3CancelBtn");
    const step3Progress = $("step3Progress");
    const step3ProgressLabel = $("step3ProgressLabel");
    const step3DownloadLink = $("step3DownloadLink");
    const step3Status = $("step3Status");
    let step3CancelRef = { cancelled: false };
    let step3Loaded = null;
    ...
    step3GenerateBtn.addEventListener("click", async () => {
        ...
        step3DownloadLink.hidden = true;
        if (!step3Loaded)
            return setStatus(step3Status, "Load a processed PNG first.", "error");
        ...
        try {
            const cubeBlob = await generateCubeLut(step3Loaded.imageData, step3Loaded.geom, (p) => {
